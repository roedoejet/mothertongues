...
class MTDConfigTemplate:
    def __init__(self, config):
        self.config_template_object = {"L1": {"name": config['config']['L1'],
                                              "lettersInLanguage": config['config']['alphabet']},
                                       "L2": {"name": config['config']['L2']}}
...
class MTDTransducerTemplate(MTDConfigTemplate):
    def __init__(self, config, cors=[], composite_transducers=[]):
        ...
        self.cors = cors
        self.keys = sorted([cor['from']
                            for cor in self.cors], key=len, reverse=True)
